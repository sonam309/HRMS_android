...
const aadharValidationSlice = createSlice({
  ...
  extraReducers: builder => {
    ...
    builder.addMatcher(
      isAnyOf(getAadharValidation.fulfilled),
      (state, action) => {
        state.loading = false;
    ...
    builder.addMatcher(
      isAnyOf(getAadharValidation.rejected),
      (state, action) => {
        state.loading = false;
      },
    );
  ...
  reducers: {
    resetAadharResult(state, action) {
      state.aadharValidationResult = {};
      state.verifyOtpResult = {};
      state.loading = false;
      state.otpLoading = false;
    },
  },
...
export const {resetAadharResult} = aadharValidationSlice.actions;
...
export const otpValidation = createAsyncThunk(
  ...
  async (payload, toolkit) => {
    return await AxiosClient('POST', `api/Kyc/VerifyOtp`, payload, toolkit);
  },
);
