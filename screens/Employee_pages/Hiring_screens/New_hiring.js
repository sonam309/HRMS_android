...
const New_hiring = (props) => {
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView>
                <View>
                    {jobOpening ? jobOpening.map((item) => <ListData title={item.DESIGNATION_NAME} opening={item.NO_OF_POSITION} comp={item.COMPENSATION} location={item.STATE_NAME} upload={item.CREATED_DATE} experience={item.MIN_EXP} JD={item.UPLOAD_JD_DOC} />) : <Text>No Data Found</Text>}
                </View>
            </ScrollView>
        </View>
    )
