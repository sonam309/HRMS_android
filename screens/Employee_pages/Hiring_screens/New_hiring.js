...
const New_hiring = props => {
  ...
  function ListData(props) {
    const title = props.title,
      positions = props.opening,
      compensation = props.comp,
      location = props.location,
      upload = props.upload,
      experience = props.experience,
      Job_Desc = props.JD,
      job_status = props.status,
      HIRING_LEAD = props.hiringLeadId,
      HIRING_LEAD_NAME = props.hiringLeadName;
    counting += 1;
    ...
    return (
      <View
        ...
        <View
          ...
          <View style={{justifyContent: 'space-between'}}>
            ...
            <View
              ...
              <Text style={{color: 'black', fontSize: 14}}>
                <Icons name="account-outline" color={COLORS.gray} size={16} />
                {positions} Opening
              </Text>
              {compensation && (
                <Text style={{color: 'black', fontSize: 14}}>
                  <Icons name="cash" color={COLORS.gray} size={16} />
                  {compensation}
                </Text>
              )}
              <Text style={{color: 'black', fontSize: 14}}>
            ...
            <View
              ...
              <Text style={{color: 'black', marginHorizontal: 2, fontSize: 14}}>
                <Icons name="account-outline" color={COLORS.gray} size={16} />
                {HIRING_LEAD !== null
                  ? HIRING_LEAD_NAME + '(' + HIRING_LEAD + ')'
                  : '-'}
              </Text>
            </View>
          </View>
        ...
        <View
          ...
          <TouchableOpacity
            onPress={() => navigation.navigate('Job_Description', {Job_Desc})}>
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Posting a new Job */}
      <TouchableOpacity
        style={styles.newJobOpeneingTxt}
        onPress={() => navigation.navigate('CreateNewJobOpening')}>
        <Text
          style={{
            color: COLORS.green,
            fontSize: 16,
      ...
      <ScrollView>
        <View>
          {selectedOption ? (
            jobOpening?.filter(items => items.JOB_STATUS === selectedOption)
              .length > 0 ? (
              jobOpening
                ?.filter(items => items.JOB_STATUS === selectedOption)
                ?.map(item => (
                  <ListData
                    key={item.TXN_ID}
                    status={item.JOB_STATUS}
                    title={item.DESIGNATION_NAME}
                    opening={item.NO_OF_POSITION}
                    comp={item.COMPENSATION}
                    location={item.STATE_NAME}
                    upload={item.CREATED_DATE}
                    experience={item.MIN_EXP}
                    JD={item.UPLOAD_JD_DOC}
                    hiringLeadId={item.HIRING_LEAD}
                  />
                ))
            ) : (
            ...
            jobOpening?.map(item => (
              <ListData
                ...
                JD={item.UPLOAD_JD_DOC}
                hiringLeadId={item.HIRING_LEAD}
                hiringLeadName={item.HIRING_LEAD_NAME}
              />
            ))
          ) : (
            <Text style={{textAlign: 'center', paddingVertical: 10}}>
