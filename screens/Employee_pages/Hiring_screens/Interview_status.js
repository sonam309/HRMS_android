...
const Interview_status = (props) => {
    ...
    function CandidateList(props) {
        ...
        const id = props.id, name = props.name, designation = props.designation, interviewStart = props.startTime, interviewEndTime = props.endTime, date = props.date

        return (
            <TouchableOpacity key={id} style={{ width: '100%', padding: 4 }} onPress={() => navigation.navigate('Candidate_details')}>
                ...
                <View style={{ borderRadius: 60, backgroundColor: COLORS.bg_tile_Colo, flexDirection: 'row', height: 80, alignItems: 'center', elevation: 6, justifyContent: 'space-evenly', marginTop: 15 }}>
                    ...
                    <View >
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
                            ...
                            <Text style={{ color: COLORS.gray, fontSize: 13, fontWeight: '500', marginHorizontal: 1 }}> {designation}
                            </Text>
                        </View>
                    ...
                    <View style={{ justifyContent: 'flex-end' }}>
                        <Text style={{ backgroundColor: '#8467D7', borderRadius: 20, textAlign: 'center', alignSelf: 'center', padding: 8, color: COLORS.white, fontWeight: '500' }}>
                            {interviewStart} - {interviewEndTime}
                        </Text>
                    </View>
        ...
        <ScrollView>
            ...
            <View style={{ flex: 1, flexDirection: 'row', padding: 8, justifyContent: 'space-between' }}>
                <ScrollView >
                    ...
                    {interViewDetail?.filter((item) => item.STATUS === status).map((item) => <CandidateList id={item.INTERVIEW_ID} name={item.CANDIDATE_NAME} designation={item.JOB_TITLE} startTime={item.SCHEDULE_TIME_FROM} endTime={item.SCHEDULE_TIME_TO} date={item.SCHEDULED_DATE} image={item.image} />)}
                </ScrollView>
            </View>
