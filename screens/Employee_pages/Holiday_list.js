    ...
    <View>
      ...
      {holidayList.length > 0 &&
        holidayList.map((holiday, index) => {
          return (
            <View style={{ backgroundColor: '#ffffff', marginVertical: 5, padding: 5, paddingHorizontal: 20, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', }} key={index}>
              <View style={{ backgroundColor: getRandomColor(), }}>
                <Text style={{ padding: 5, color: '#ffffff', fontSize: 20, }}> {holiday.DATED}</Text>
              </View>
              <View>
                <Text style={{ fontSize: 16, color: 'black', }}>   {holiday?.DESCR} </Text>
                <Text style={{ fontSize: 12, color: 'gray', }}>     {holiday?.DAY_NAME}
                </Text>
              </View>
            </View>
          );
