...
const SkillsBottomView = ({skills, onPress, fetchSkillsData}) => {
  ...
  const getDropdownData = async P => {
    ...
    if (P === 39) {
      setSkillLevelDropDown(returnedData);
    }
  };
  ...
  const checkSkillValue = value => {
    for (let index = 0; index < skillLevelDropDown.length; index++) {
      ...
      if (element.PARAM_NAME === value)
        setSelectedSkillLevelValue(element.PARAM_ID);
    }
  };
    ...
    return (
      <View
        ...
        <Text style={{fontWeight: 600}}>
          Skill Level:-{' '}
          <Text style={{fontWeight: 400}}>{item.SKILL_LEVEL}</Text>
        </Text>
        <Text style={{fontWeight: 600}}>
          Total Experience:-{' '}
          <Text style={{fontWeight: 400}}>{item.TOTAL_EXP}</Text>
        </Text>
      </View>
    );
  };
  ...
  const selectDropDownValue = id => {
    if (id === 'skill') {
      return selectedSkillLevelValue
        ? selectedSkillLevelValue
        : skillLevelDropDown?.map(a => a.PARAM_NAME)[0];
    }
  };
          ...
          {showSkills && allSkills[0]?.FLAG === 'S' ? (
            ...
            <View>
              ...
              <View
                ...
                <Text style={{color: 'green', paddingHorizontal: 6}}>
                  Skill
                </Text>
                <TextInput
                  value={skill}
