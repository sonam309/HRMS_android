    ...
    const DisplayQualification = ({ item, key }) => {
        return (
            <View key={key} style={{ backgroundColor: COLORS.disableOrange1, padding: 6, borderRadius: 12, marginVertical: 4 }}>
                ...
                <Text style={{ fontWeight: 600 }}>Institute:- <Text style={{ fontWeight: 400 }}>{item.INSTITUTE}</Text></Text>
                <Text style={{ fontWeight: 600 }}>Mode:- <Text style={{ fontWeight: 400 }}>{item.QUALIFICATION_MODE}</Text></Text>
                <Text style={{ fontWeight: 600 }}>Batch:- <Text style={{ fontWeight: 400 }}>{item.FROM_YEAR} - {item.PASS_YEAR}</Text></Text>
                <Text style={{ fontWeight: 600 }}>Location:- <Text style={{ fontWeight: 400 }}>{item.CITY}, {item.STATE_NAME}, {item.COUNTRY}</Text></Text>
            </View>
        )
    }
