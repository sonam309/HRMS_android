...
const BankBottomView = ({ onPress }) => {
  ...
  const getDropdownData = async P => {
    ...
    if (P === 36) {
      setBank(returnedData);
    } else if (P === 27) {
      setAccountType(returnedData);
    } else {
      setOperation(returnedData);
    }
  };
  ...
  const checkSelectedOperation = value => {
    for (let index = 0; index < operation.length; index++) {
      ...
      if (element.PARAM_NAME === value)
        setSelectedOperationValue(element.PARAM_ID);
    }
  };
  ...
  const checkSelectedAccountType = value => {
    for (let index = 0; index < accountType.length; index++) {
      ...
      if (element.PARAM_NAME === value)
        setSelectedAccountTypeValue(element.PARAM_ID);
    }
  };
  ...
  const selectDropDownValue = id => {
    if (id === 'accountType') {
      return selectedAccountTypeValue
        ? selectedAccountTypeValue
        : accountType?.map(a => a.PARAM_ID)[0];
    } else if (id === 'bankName') {
      return selectedBankValue
        ? selectedBankValue
        : bank?.map(a => a.PARAM_ID)[0];
    } else
      return selectedOperationValue
        ? selectedOperationValue
        : operation?.map(a => a.PARAM_ID)[0];
  };
  ...
  const selectDropDownText = id => {
    if (id === 'accountType') {
      return selectedAccountType
        ? selectedAccountType
        : accountType?.map(a => a.PARAM_NAME)[0];
    } else if (id === 'bankName') {
      return selectedBank ? selectedBank : bank?.map(a => a.PARAM_NAME)[0];
    } else
      return selectedOperation
        ? selectedOperation
        : operation?.map(a => a.PARAM_NAME)[0];
  };
          ...
          {showDoc != '' ? (
            ...
            <KeyboardAwareScrollView
              ...
              {/* <Text>
                ...
                {console.log("first",JSON.stringify(selectedBank))}
              </Text>  */}
              <TextDropdown
                caption={'Bank Name'}
                data={bank}
              ...
              <TextInput
                value={cAccountNo}
                onChangeText={val => (
                  setCAccountNo(val), setCheckAccountError(true)
                )}
                style={styles.inputHolder}
                keyboardType="numeric"
                maxLength={20}
              ...
              <TextInput
                value={accountHolder}
                onChangeText={val => setAccountHolder(val)}
                style={styles.inputHolder}
              />
