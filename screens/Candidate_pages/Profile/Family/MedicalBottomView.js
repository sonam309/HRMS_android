  ...
  const saveMedicalPolicyDetails = async () => {
    ...
    try {
      ...
      let medicalData = {
        ...
        candidateId: userId,
        userId: userId,
        medicalPolicyName: policyName,
        medicalPolicynumber: policyNumber,
        medicalMembershipNumber: membershipNumber,
        medicalPolicyCategory: policyCategory,
        policyExpiry: selectedExpiryDate,
      };
  ...
  const PolicyDetails = () => {
    return (
      <View style={{padding: 4}}>
        ...
        {medicalPolicy.map((item, index) => (
          <DisplayPolicy item={item} key={index} />
        ))}
      </View>
    );
  };
    ...
    return (
      <View
        ...
        <Text style={{fontWeight: 600}}>
          ...
          <Text style={{fontWeight: 400}}>{item?.MEDICAL_POLICY_NUMBER}</Text>
        </Text>
        <Text style={{fontWeight: 600}}>
          Medical Membership Number:-{' '}
          <Text style={{fontWeight: 400}}>
            {item?.MEDICAL_MEMBERSHIP_NUMBER}
          </Text>
        </Text>
        <Text style={{fontWeight: 600}}>
          Medical Policy Category:-{' '}
          <Text style={{fontWeight: 400}}>{item?.MEDICAL_POLICY_CATEGORY}</Text>
        </Text>
        <Text style={{fontWeight: 600}}>
          Medical Policy Expiry:-{' '}
          <Text style={{fontWeight: 400}}>{item?.MEDICAL_POLICY_EXPIRY}</Text>
        </Text>
      </View>
    );
  };
      ...
      {loaderVisible ? (
        ...
        <KeyboardAwareScrollView
          ...
          {/* <ScrollView style={{ height: '100%' }} showsVerticalScrollIndicator={false}> */}
          {
            ...
            !showForm &&
            medicalPolicy[0]?.MEDICAL_POLICY_NUMBER &&
            medicalPolicy.length >= 0 ? (
              <PolicyDetails />
            ) : (
              <View>
                {/* Policy Name */}
                ...
                <TextInput
                  value={policyName}
                  onChangeText={val => setPolicyName(val)}
                  style={[
                    styles.inputHolder,
                ...
                <TextInput
                  ...
                  onChangeText={val => setMembershipNumber(val)}
                  style={[
                    styles.inputHolder,
                    {marginVertical: 3, marginHorizontal: 7},
                  ]}
                ...
                <TextInput
                  ...
                  onChangeText={val => setPolicyCategory(val)}
                  style={[
                    styles.inputHolder,
                    {marginVertical: 3, marginHorizontal: 7},
                  ]}
                ...
                <View style={{flexDirection: 'row', margin: 3}}>
                  <TextInput
                    style={[
                      styles.inputHolder,
                      {width: '48%', margin: 3, color: COLORS.black},
                    ]}
                  ...
                  <DatePicker
                    ...
                    mode="date"
                    date={expiryDate}
                    onConfirm={date => SelectExpiryDate(date)}
                    onCancel={() => {
                      setCalendarOpen(false);
                    }}
                  />
