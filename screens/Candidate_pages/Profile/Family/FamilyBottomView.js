...
const FamilyBottomView = ({ members, setMembers, onPress, fetchFamilyData }) => {
    ...
    const selectDropDownValue = (id) => {
        if (id === "familyMember") {
            return selectedFamilyMemberValue ? selectedFamilyMemberValue : familyMemberDropDown?.map(a => a.PARAM_ID)[0];
        }
    ...
    const MemberDetails = () => {
        return (
            <View style={{ padding: 4 }}>
                ...
                {
                    members[0]?.MEMBER_FIRST_NAME && members.map((item) => <DisplayMember item={item} key={item.FAMILY_MEMBER} />)
