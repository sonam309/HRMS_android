...
function generateRandom12DigitNumber() {
    ...
    const randomNumber = Math.floor(Math.random() * 9000000000000) + 1000000000000;
 ...
 try {
    let res = await fetch(`${API}/api/addCandidate`, {
      ...
      if(finalRes){
        ...
        navigation.goBack()
        Alert.alert(`Your Candidate ID is ${finalRes?.Result[0]?.CANDIDATE_ID}, Please check you mail box for details.`,[
            {text: 'OK'}
        ])
      }
